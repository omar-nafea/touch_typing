...
const TheMistakes = document.querySelector(".mistakes span");
const TheTime = document.querySelector(".time span");
const theWPM = document.querySelector(".wpm span");
const theCpm = document.querySelector(".cpm span");
const reset = document.querySelector(".reset");
const stars = document.querySelectorAll(".Evaluation .star");
const yourWPM = document.querySelector(".your_WPM");
...
const paragraphs_2 = [
    "This could be, or perhaps few can name a pasteboard quiver that isn't a brittle alligator. A swordfish is a death's numeric. Authors often misinterpret the mist as a swelling asphalt, when in actuality it feels.",
    "The vision of an attempt becomes a lawny output. Dibbles are mis womens. The olden penalty reveals itself as a bustled field to those who look. Few can name a chalky force.",
    ...
    "The frosts could be said to resemble backstage chards. One cannot separate colleges from pinkish bacons. Far from the truth, the mom of a rooster becomes a chordal hydrogen. A tempo can hardly be considered a purer credit.",
...
function generateParagraph() {
    ...
    paragraph.innerHTML = "";
    let theLength = para[random].split("");
    theLength.forEach(char => {
...
function inputingDiff() {
    ...
    if (charIndex < characters.length - 1) {
        ...
        if (typedChar == null) {
            ...
            charIndex++
            if (characters[charIndex].innerHTML === " ") {
                space.style.opacity = 1;
            } else {
                space.style.opacity = 0;
            }
        }
        TheMistakes.innerHTML = mistakes
        characters.forEach((char) => { char.classList.remove('active') });
        characters[charIndex].classList.add('active');
        Wpm = Math.floor((((charIndex - mistakes) / 5) / (timeLeft - maxTime)) * 60)
        Wpm = Wpm < 0 || !Wpm || Wpm === Infinity ? 0 : Wpm
        theWPM.innerHTML = Wpm
        theCpm.innerHTML = charIndex - mistakes
    } else {
        input.value = "";
        if (Wpm > 60) {
            stars.forEach((str, i) => {
        ...
        yourWPM.innerHTML = `<p>Congratulations! Your Wpm is <span>${Wpm}</span></p>`;
        if (Wpm < 15) {
            yourWPM.innerHTML = "<p>sorry you did't pass the requirements</p>"
        }
        clearInterval(timer);
    }
}
...
input.addEventListener("input", inputingDiff)
reset.addEventListener('click', resetThegame)
window.onload = function() { generateParagraph() }